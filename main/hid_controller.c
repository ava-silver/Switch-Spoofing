...
void hid_queue_command(hid_controller_t *controller, const hid_command_t *command, uint8_t *arg, command_response_cb response_cb, uint8_t response_required)
{
    ...
    if (arg)
    {
        ...
        if ((cur_command->arg_offset + cur_command->arg_len) > cur_command->len)
            cur_command->len += (cur_command->arg_offset + cur_command->arg_len - cur_command->len);
    }
...
uint8_t hid_send_next_command(hid_controller_t *controller)
{
    if (controller->command_queue_num)
    {
        ...
        case CNT_JOYCON_L:
        case CNT_PROCON:
            if (command->cmd[1] == 0x01 || command->cmd[1] == 0x10)
            { // Check if subcommand or rumble command
                ...
                if (controller->rumble)
                    memcpy(command->cmd + 3, rumble_on_data, 8);
                else
                    memcpy(command->cmd + 3, rumble_off_data, 8);
            }
            break;
        case CNT_WIIMOTE:
        case CNT_WIIU_PRO:
            if (controller->rumble)
                command->cmd[2] |= 0x01; // Turn rumble on
            else
                command->cmd[2] &= 0xFE; // Turn rumble off
            break;
        default:
            break;
        }
...
void hid_get_response(hid_controller_t *controller, uint8_t *response, uint8_t response_size)
{
    ...
    case CNT_WIIMOTE:
        ...
        if ((response[1] == 0x37) && (response[22] != 0xFF) && (controller->wmp_type == WMP_SUPPORTED) && controller->wmp_active)
        {                                                                          // If byte 22 is 0xFF then WMP is not sending data yet
...
uint8_t hid_response_matches_command(hid_controller_t *controller, hid_command_t *command)
{
    ...
    case CNT_PROCON:
        ...
        case READ_CAL_JOY_R_USER:
            if (!memcmp(controller->command_response + 15, command->cmd + 11, 6))
                return 1;
            break;
        // Standard subcommands
        default:
            if (!memcmp(controller->command_response + 15, command->cmd + 11, 1))
                return 1;
            break;
        }
        break;
    ...
    case CNT_WIIU_PRO:
        ...
        // Status command
        case GET_STATUS:
            if (controller->command_response[1] == 0x20)
                return 1;
            break;
        ...
        case READ_CAL_WMP:
        case READ_CAL_ACCEL:
            if ((controller->command_response[1] == 0x21) && !memcmp(controller->command_response + 5, command->cmd + 4, 2))
                return 1;
            break;
        // Standard command response
        default:
            if ((controller->command_response[1] == 0x22) && (controller->command_response[4] == command->cmd[1]))
                return 1;
            break;
        }
        break;
...
void hid_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
    ...
    case L2CAP_DATA_PACKET:
        ...
        if (controller)
            hid_get_response(controller, packet, size);
        break;
    default:
        break;
    }
...
{
    if (controller_num > 0 && controller_num <= 8)
        return &controllers[controller_num - 1];
    return NULL;
}
...
hid_controller_t *get_controller_from_cid(uint16_t cid)
{
    ...
    for (i = 0; i < 8; i++)
    {
        if (controllers[i].registered && (controllers[i].l2cap_control_cid == cid || controllers[i].l2cap_interrupt_cid == cid))
...
hid_controller_t *get_controller_from_addr(uint8_t *addr)
{
    ...
    for (i = 0; i < 8; i++)
    {
        if (controllers[i].registered && !memcmp(controllers[i].address, addr, 6))
            return &controllers[i];
    }
    return NULL;
}
    ...
    for (i = 0; i < 8; i++)
    {
        if (controllers[i].gamepad_num == gamepad_num)
        {
            ...
            case CNT_WIIU_PRO:
                ...
                {
                    if (controllers[i].connected && controllers[i].registered)
                        *controller_main = &controllers[i];
                }
                else if (controllers[i].registered)
                    *controller_main = &controllers[i];
                break;
            case CNT_JOYCON_L:
                if (connection_required)
                {
                    if (controllers[i].connected && controllers[i].registered)
                        *controller_secondary = &controllers[i];
                }
                else if (controllers[i].registered)
                    *controller_secondary = &controllers[i];
                break;
...
{
    if (get_controller_from_addr(addr))
        return NULL; // Controller with this address already registered

    if (controller_type == CNT_NONE)
        controller_type = get_controller_type(addr);
    else
        store_controller_type(controller_type, addr);
    if (controller_type == CNT_NONE)
        return NULL; // No controller type found for this address
    ...
    {
        if ((gamepad_num == UART_GAMEPAD_NUM) && (joycon_right.data_ready || joycon_left.data_ready))
            gamepad_num++; // Skip this gamepad if wired Joy-Con are connected to it
        get_controllers_from_gamepad(gamepad_num, &controller_main, &controller_secondary, 0);
        if (!controller_main && !controller_secondary)
            break; // No controllers for this gamepad slot
        if (!controller_main)
        {
    ...
    for (i = 0; i < 8; i++)
    {
        if (!controllers[i].registered && gamepad_num <= 4)
        {
            ...
            if (!is_second_joycon)
                gamepad_reset(&gamepads[gamepad_num - 1]); // Do not erase gamepad settings if another Joy-Con is actively using it
...
void controller_setup_extension(hid_controller_t *controller)
{
    if (!memcmp(controller->command_response + 7, ext_id_nunchuk, 6))
    {
        ...
        if (controller->wmp_type == WMP_SUPPORTED)
            hid_queue_command(controller, &cmd_wiimote_activate_wmp_passthrough_nunchuk, NULL, &controller_activate_motion_plus_cb, 1);
    }
    else if (!memcmp(controller->command_response + 7, ext_id_classic, 6) || !memcmp(controller->command_response + 7, ext_id_classic_pro, 6))
    {
        ...
        if (controller->wmp_type == WMP_SUPPORTED)
            hid_queue_command(controller, &cmd_wiimote_activate_wmp_passthrough_classic, NULL, &controller_activate_motion_plus_cb, 1);
    }
    else
    {
...
void controller_set_leds(hid_controller_t *controller, uint8_t player_num)
{
    if (controller->player_num != player_num)
    {
        ...
        case CNT_PROCON:
            ...
            {
            case 1:
                controller->led_state = 0x01;
                break;
            case 2:
                controller->led_state = 0x03;
                break;
            case 3:
                controller->led_state = 0x07;
                break;
            case 4:
                controller->led_state = 0x0F;
                break;
            default:
                controller->led_state = 0x00;
                break;
            }
        ...
        case CNT_WIIU_PRO:
            ...
            {
            case 1:
                controller->led_state = 0x12;
                break;
            case 2:
                controller->led_state = 0x22;
                break;
            case 3:
                controller->led_state = 0x42;
                break;
            case 4:
                controller->led_state = 0x82;
                break;
            default:
                controller->led_state = 0x02;
                break;
            }
...
void controller_rumble(hid_controller_t *controller, uint8_t enable)
{
    ...
    case CNT_PROCON:
        if (!controller->command_queue_num)
            hid_queue_command(controller, &cmd_joycon_rumble, NULL, NULL, 0);
        break;
    case CNT_WIIMOTE:
    case CNT_WIIU_PRO:
        if (!controller->command_queue_num)
            hid_queue_command(controller, &cmd_wiimote_rumble, NULL, NULL, 0);
        break;
    default:
        break;
    }
...
void controller_handle(uint8_t controller_num)
{
    ...
    if (controller->connected)
    {
        ...
        if (controller->time_connected == 0)
            controller->time_connected = app_timer;
        ...
        if (!controller->command_send_event_queued)
        {
            ...
            if (prev_command->response_required)
            {
                if (controller->command_response_verified)
                {
                    ...
                    if (hid_send_next_command(controller))
                        controller->command_response_verified = 0;
                }
            }
            else
                hid_send_next_command(controller);
        }
    }
}
...
void controller_setup(hid_controller_t *controller)
{
    ...
    case CNT_WIIMOTE:
        if (controller->pairing)
            end_pair(controller); // Pin code pairing occurs in HCI event handler
        // hid_queue_command(controller, &cmd_wiimote_report_mode_acc_ir12, NULL, NULL, 1);
...
{
    if (!controller->pairing)
        gap_disconnect(controller->handle);
}