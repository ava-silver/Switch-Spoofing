...
static uint8_t wiimote_joy_to_ir(wiimote_t *wiimote, gamepad_t *gamepad)
{
    ...
    if (joy_x_sq + joy_y_sq > deadzone_sq)
    { // Joystick is outside deadzone
        ...
        if (wiimote->ir_object[0].x > 0x3FF)
            wiimote->ir_object[0].x = 0x3FF;
        if (wiimote->ir_object[0].x < 0)
            wiimote->ir_object[0].x = 0;
        if (wiimote->ir_object[0].y > 0x3FF)
            wiimote->ir_object[0].y = 0x3FF;
        if (wiimote->ir_object[0].y < 0)
            wiimote->ir_object[0].y = 0;
        if (wiimote->ir_object[1].x > 0x3FF)
            wiimote->ir_object[1].x = 0x3FF;
        if (wiimote->ir_object[1].x < 0)
            wiimote->ir_object[1].x = 0;
        if (wiimote->ir_object[1].y > 0x3FF)
            wiimote->ir_object[1].y = 0x3FF;
        if (wiimote->ir_object[1].y < 0)
            wiimote->ir_object[1].y = 0;

        return 1;
    }
    else
    {
        if (!wiimote->ir_idle && ((app_timer - wiimote->ir_idle_timeout) >= IR_JOY_IDLE_TIME))
            wiimote->ir_idle = 1;
        return 0;
    }
}
...
static void wiimote_update_ir(wiimote_t *wiimote, gamepad_t *gamepad)
{
    ...
    case GAMEPAD_PROCON:
        ...
        {
            if (!wiimote_joy_to_ir(wiimote, gamepad))
                wiimote_angle_to_ir(wiimote, gamepad); // Use angles if joystick is inactive
        }
        else
            wiimote->ir_idle = 1;
        break;
...
void wiimote_set_extension(uint8_t wiimote_num, enum EXTENSION_TYPE extension)
{
    if (wiimote_num > 0 && wiimote_num <= 4)
        wiimotes[wiimote_num - 1].extension = extension;
}
...
void wiimote_change_extension(uint8_t wiimote_num)
{
    ...
    {
    case GAMEPAD_JOYCON_DUAL:
        if (wiimote->extension == EXT_NUNCHUK)
            wiimote->extension = EXT_CLASSIC;
        else if (wiimote->extension == EXT_CLASSIC)
            wiimote->extension = EXT_NUNCHUK;
        else
            wiimote->extension = EXT_NUNCHUK;
        break;
    case GAMEPAD_PROCON:
...
void wiimote_spi_send(uint8_t wiimote_num)
{
    ...
    if (wiimote->active)
    {
        ...
        for (i = 0; i < 4; i++)
            checksum_buttons += buf[i + 1];
        checksum_buttons += 0x55;
        for (i = 0; i < 25; i++)
            checksum_axes += buf[i + 5];
        checksum_axes += 0x55;
...
void wiimote_spi_receive()
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        if (wiimote_num > 0 && wiimote_num <= 4)
        {
            ...
            if (controller_main && !connection_allowed)
                controller_disconnect(controller_main);
            if (controller_secondary && !connection_allowed)
                controller_disconnect(controller_secondary);
