...
static void uart_joycon_write_next_command(uart_joycon_t *joycon)
{
    ...
    for (i = 1; i <= uart_handshake_commands[joycon->handshake_pos][0]; i++)
        joycon->uart_port->fifo.rw_byte = uart_handshake_commands[joycon->handshake_pos][i];
...
static void uart_joycon_intr_handle(uart_joycon_t *joycon)
{
    if (!joycon->chars_remaining)
        gpio_set_level(joycon->rx_en_pin, 0);
    ...
    if (joycon->message_started)
    {
        ...
        if (joycon->chars_remaining > 0)
            joycon->uart_intr.rxfifo_full_thresh = joycon->chars_remaining;
        else
            joycon->chars_remaining = 8; // Assuming 8 chars remaining
    }
    else
    {
        joycon->message_received = 1;
        joycon->uart_intr.rxfifo_full_thresh = 4; // Wait for next command header
        if (!gpio_get_level(joycon->tx_en_pin))
            uart_joycon_write_next_command(joycon);
        else
            joycon->command_queued = 1;
        if (!memcmp(joycon->rx_buf, uart_response_baud_switched, 12))
            joycon->baud_switch_queued = 1;
    }
...
void uart_store_link_key(uart_joycon_t *joycon)
{
    if (!joycon->key_stored)
    {
        ...
        uint8_t link_key[16];
        for (i = 0; i < 16; i++)
            link_key[i] = joycon->rx_buf[43 - i] ^ 0xAA; // Read link key little-endian, XOR each byte with 0xAA
        printf("Link key: ");
...
void uart_joycon_handle(uart_joycon_t *joycon)
{
    if (joycon->connected)
    {
        if (joycon->message_received)
        {
            ...
            if (!memcmp(joycon->rx_buf + 26, uart_handshake_commands[7] + 23, 6))
                uart_set_cal_imu(joycon);
            if (!memcmp(joycon->rx_buf + 26, uart_handshake_commands[8] + 23, 6) && joycon->type == UART_JOYCON_L)
                uart_set_cal_joy(joycon);
            if (!memcmp(joycon->rx_buf + 26, uart_handshake_commands[9] + 23, 6) && joycon->type == UART_JOYCON_R)
                uart_set_cal_joy(joycon);
            if (!memcmp(joycon->rx_buf + 26, uart_handshake_commands[10] + 23, 2))
                uart_set_mac_address(joycon);
            if (!memcmp(joycon->rx_buf + 26, uart_handshake_commands[11] + 23, 2))
                uart_store_link_key(joycon);

            if (!memcmp(joycon->rx_buf, uart_status_response_header, 8))
                joycon->data_ready = 1;
            else
                joycon->data_ready = 0;
        ...
        if ((app_timer - joycon->connect_timer > 500) && gpio_get_level(joycon->tx_en_pin))
