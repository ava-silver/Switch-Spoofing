    ...
    if (hid_joycon_right)
    {
        ...
        case 0x30:
            ...
            if (gamepad->axes.joy_rx <= gamepad->cal.joy_rx_center)
                ...
                gamepad->axes.joy_rx = map(gamepad->axes.joy_rx, gamepad->cal.joy_rx_center + 1, gamepad->cal.joy_rx_max, 2048, 4095);
            ...
            if (gamepad->axes.joy_ry <= gamepad->cal.joy_ry_center)
                ...
                gamepad->axes.joy_ry = map(gamepad->axes.joy_ry, gamepad->cal.joy_ry_center + 1, gamepad->cal.joy_ry_max, 2048, 4095);
            break;
        case 0x3F:
            break;
        default:
    ...
    if (hid_joycon_left)
    {
        ...
        case 0x30:
            ...
            if (gamepad->axes.joy_lx <= gamepad->cal.joy_lx_center)
                ...
                gamepad->axes.joy_lx = map(gamepad->axes.joy_lx, gamepad->cal.joy_lx_center + 1, gamepad->cal.joy_lx_max, 2048, 4095);
            ...
            if (gamepad->axes.joy_ly <= gamepad->cal.joy_ly_center)
                ...
                gamepad->axes.joy_ly = map(gamepad->axes.joy_ly, gamepad->cal.joy_ly_center + 1, gamepad->cal.joy_ly_max, 2048, 4095);
            break;
        case 0x3F:
            break;
        default:
...
static void gamepad_parse_uart_input(gamepad_t *gamepad)
{
    if (joycon_right.data_ready)
    {
        ...
        if (gamepad->axes.joy_rx <= joycon_right.joy_x_center)
            ...
            gamepad->axes.joy_rx = map(gamepad->axes.joy_rx, joycon_right.joy_x_center + 1, joycon_right.joy_x_max, 2048, 4095);
        ...
        if (gamepad->axes.joy_ry <= joycon_right.joy_y_center)
            ...
            gamepad->axes.joy_ry = map(gamepad->axes.joy_ry, joycon_right.joy_y_center + 1, joycon_right.joy_y_max, 2048, 4095);
    }
    if (joycon_left.data_ready)
    {
        ...
        if (gamepad->axes.joy_lx <= joycon_left.joy_x_center)
            ...
            gamepad->axes.joy_lx = map(gamepad->axes.joy_lx, joycon_left.joy_x_center + 1, joycon_left.joy_x_max, 2048, 4095);
        ...
        if (gamepad->axes.joy_ly <= joycon_left.joy_y_center)
            ...
            gamepad->axes.joy_ly = map(gamepad->axes.joy_ly, joycon_left.joy_y_center + 1, joycon_left.joy_y_max, 2048, 4095);
    }
}
...
static uint8_t gamepad_gyro_calibrate(gamepad_t *gamepad)
{
    if (gamepad->calibrating)
    {
        ...
        {
            if (app_timer - gamepad->calibration_timer >= 3000)
            { // Start calibration after 3s
...
static void gamepad_gyro_handle(gamepad_t *gamepad, hid_controller_t *controller)
{
    ...
    else
    {
        ...
        { // Wiimote uses long-press to center and long-press to calibrate
            if (gamepad->gyro_center.triggered)
            {
                gamepad_reset_angles(gamepad);
                if (controller)
                    controller_rumble_pattern(controller, 70, 0, 1);
            ...
            if (gamepad->gyro_calibrate.triggered)
            {
                ...
                if (controller)
                    controller_rumble_pattern(controller, 100, 500, 5);
                gamepad->gyro_calibrate.triggered = 0;
            }
            // Add option to toggel gyro/IR camera with gyro_enable
        }
        else
        { // Other controllers use short-press to center and long-press to calibrate
        ...
        if (controller_main)
        {
            ...
            case CNT_JOYCON_R:
                if (controller_secondary)
                    gamepad->type = GAMEPAD_JOYCON_DUAL;
                else
                    gamepad->type = GAMEPAD_JOYCON_R_VERT;
                break;
            case CNT_PROCON:
            ...
            gamepad->type = GAMEPAD_JOYCON_L_VERT;
    }
}
...
void gamepad_get_angles_from_controller(hid_controller_t *controller)
{
    ...
    if (!gamepad->calibrating)
    {
        ...
        ESP_ERROR_CHECK(gptimer_get_raw_count(gptimer, &cur_time));
    ...
    if (gamepad_num > 0 && gamepad_num <= 4)
    {
        ...
        else
        {
            ...
            case GAMEPAD_JOYCON_DUAL:
                ...
                if (wiimotes[gamepad_num - 1].extension != EXT_CLASSIC)
                    gamepad_gyro_handle(gamepad, controller_main);
                break;
            case GAMEPAD_JOYCON_L_VERT:
        ...
        {
            if (controller_main)
                wiimote_set_extension(gamepad_num, controller_main->extension_type);
        }
        else
        {
            if (gamepad->mode_switch.triggered)
            {
                ...
                if (controller_main)
                    controller_rumble_pattern(controller_main, 100, 70, 2);
                if (controller_secondary)
                    controller_rumble_pattern(controller_secondary, 100, 70, 2);
...
void gamepad_set_cal_accel_r(hid_controller_t *controller)
{
    ...
    case CNT_PROCON:
        ...
        {
        case 0:
            if ((controller->command_response[21] == 0xB2) && (controller->command_response[22] == 0xA1))
            { // Magic identifier for user calibration
...
void gamepad_set_cal_accel_l(hid_controller_t *controller)
{
    ...
    case CNT_WIIMOTE:
        for (i = 0; i < 14; i++)
            checksum += controller->command_response[7 + i];
        checksum += 0x55;
